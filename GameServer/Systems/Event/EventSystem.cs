...
public async Task Emit(GameEventType eventType)
{
    ...
    _logger.LogInformation("Event {EventType} emitted", eventType);
}
...
private static ImmutableDictionary<GameEventType, Func<ModelManager, Task>> RegisterModelManagerEvents()
{
    ...
    foreach (MethodInfo method in typeof(ModelManager).GetMethods(BindingFlags.Public | BindingFlags.Instance))
    {
        GameEventAttribute? attribute = method.GetCustomAttribute<GameEventAttribute>();
        if (attribute == null) continue;

        ParameterExpression modelManagerParam = Expression.Parameter(typeof(ModelManager), "modelManager");
